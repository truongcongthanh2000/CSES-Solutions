...
void sol() {
    ...
    rmq<int> st(id_rank, [&](int a, int b) {
        return min(a, b);
    });
    ...
    auto get_right = [&](const string& pattern) {
        // max i valid s[id_rank(i)..] >= pattern and contain pattern
        int L = 0, R = n - 1, ans = -1;
        while (L <= R) {
            int mid = (L + R) >> 1;
            if (cmp(id_rank[mid], pattern, 0) <= 0) {
                L = mid + 1;
                ans = mid;
            } else {
                R = mid - 1;
